...
collatz:
  ...
  collatz_loop:
    ...
    beq collatz_end            /* if x1 == 1 branch to collatz_end */
    and x2, x1, #1             /* x2 � x1 & 1 */
    cmp x2, #0                 /* compare x2 and 0 */
    bne collatz_odd            /* if x2 != 0 (this is x1 % 2 != 0) branch to collatz_odd */
  collatz_even:
    mov x1, x1, ASR #1         /* x1 � x1 >> 1. This is x1 � x1/2 */
    b collatz_end_loop         /* branch to collatz_end_loop */
  collatz_odd:
    add x1, x1, x1, LSL #1     /* x1 � x1 + (x1 << 1). This is x1 � 3*x1 */
    add x1, x1, #1             /* x1 � x1 + 1. */
  collatz_end_loop:
    add x0, x0, #1             /* x0 �x00 + 1 */

...
main:
    ...
    ldr x4, addr_mat_C  /* x4 ← c */

    vldr s0, [x4] /* s0 ← *x4. This is s0 ← c[0][0] */
    vcvt.f64.f32 d1, s0 /* Convert it into a double-precision
                           d1 ← s0
                         */
    vmov x2, x3, d1      /* {x2,x3} ← d1 */

    mov x6, sp     /* Remember the stack pointer, we need it to restore it back later */
                   /* x6 ← sp */

    mov x5, #1  /* We will iterate from 1 to 15 (because the 0th item has already been handled */
    add x4, x4, #60 /* Go to the last item of the matrix c, this is c[3][3] */
    .Lloop:
        vldr s0, [x4] /* s0 ← *x4. Load the current item */
        vcvt.f64.f32 d1, s0 /* Convert it into a double-precision
                               d1 ← s0
                             */
        sub sp, sp, #8      /* Make room in the stack for the double-precision */
        vstr d1, [sp]       /* Store the double precision in the top of the stack */
        sub x4, x4, #4      /* Move to the previous element in the matrix */
        add x5, x5, #1      /* One more item has been handled */
        cmp x5, #16         /* if x5 != 16 go to next iteration of the loop */
        bne .Lloop

    ldr x0, addr_format_result /* x0 ← &format_result */
    bl printf /* call printf */
    mov sp, x6  /* Restore the stack after the call  */

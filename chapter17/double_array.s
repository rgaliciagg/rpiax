...
double_array : 
    ...
    */
    ...
    .Lloop_array_double:
      ldr x5, [x1, x4, LSL #2]   /* x5 ← *(x1 + x4 * 4) */
      mov x5, x5, LSL #1         /* x5 ← x5 * 2 */
      str x5, [x1, x4, LSL #2]   /* *(x1 + x4 * 4) ← x5 */
      add x4, x4, #1             /* x4 ← x4 + 1 */
    .Lcheck_loop_array_double:
      cmp x4, x0                 /* x4 - x0 and update cpsr */
      bne .Lloop_array_double       /* if x4 != x0 go to .Lloop_array_double */
...
print_each_item:
    push {x4, x5, x6, x7, x8, lr} /* x8 is unused */

    mov x4, #0      /* x4 ← 0 */
    mov x6, x0      /* x6 ← x0. Keep x0 because we will overwrite it */
    mov x7, x1      /* x7 ← x1. Keep x1 because we will overwrite it */


    b .Lcheck_loop_print_items
    .Lloop_print_items:
      ldr x5, [x7, x4, LSL #2]   /* x5 ← *(x7 + x4 * 4) */

      /* Prepare the call to printf */
      ldr x0, address_of_message /* first parameter of the call to printf below */
      mov x1, x4      /* second parameter: item position */
      mov x2, x5      /* third parameter: item value */
      bl printf       /* call printf */

      add x4, x4, #1             /* x4 ← x4 + 1 */
    .Lcheck_loop_print_items:
      cmp x4, x6                 /* x4 - x6 and update cpsr */
      bne .Lloop_print_items       /* if x4 != x6 goto .Lloop_print_items */
...
main:
    push {x4, lr}
    /* we will not use x4 but we need to keep the function 8-byte aligned */

    /* first call print_each_item */
    mov x0, #256                   /* first_parameter: number of items */
    ldr x1, address_of_big_array   /* second parameter: address of the array */
